...
{ 
  ...
  CUTRY(cudaMalloc(&out->dev_shape  ,sizeof(size_t)* ndndim(out)   ));
  CUTRY(cudaMalloc(&out->dev_strides,sizeof(size_t)*(ndndim(out)+1)));
  CUTRY(cudaMalloc(&out->vol.data   ,out->dev_cap=ndnbytes(out)));
